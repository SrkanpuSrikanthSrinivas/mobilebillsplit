...
function coerceLine(v) {
  if (v == null) return { t: 0, d: 0 };
  if (typeof v === "number") return { t: v, d: 0 };
  if (typeof v === "object") return { t: Number(v.t || 0), d: Number(v.d || 0) };
  const n = Number(v);
  return { t: isFinite(n) ? n : 0, d: 0 };
}
function toLines(v) {
  let x = v;
  ...
  const out = {};
  for (const k of Object.keys(x)) out[k] = coerceLine(x[k]);
  return out;
}
